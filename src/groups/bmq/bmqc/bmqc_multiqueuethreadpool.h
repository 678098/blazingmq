...
class MultiQueueThreadPoolEvent {
  public:
    ...
    enum Type {
        ...
        /// For events that were enqueued on multiple
        ...
        /// Automatically generated when no more items are available in a queue
        BMQC_QUEUE_EMPTY,
  ...
  public:
    ...
    explicit MultiQueueThreadPoolEvent(Type              type,
...
class MultiQueueThreadPoolConfig {
  ...
  private:
    ...
    typedef MonitoredQueue<bdlcc::SingleConsumerQueue<EventPtr> > Queue;
...
class MultiQueueThreadPool BSLS_KEYWORD_FINAL {
  public:
    ...
    typedef MultiQueueThreadPoolEvent<TYPE>  Event;
    typedef typename Config::EventPtr        EventPtr;
...
inline void MultiQueueThreadPool<TYPE>::processMonitorEvents()
{
    ...
    for (size_t i = 0; i < d_queues.size(); ++i) {
        ...
        if (prevState == e_MONITOR_PENDING) {
            ...
            // Enqueue the next monitor event
                                                     ...
                                                     bslmt::Latch* latch_p)
{
    ...
    while (true) {
        ...
        if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(event == &d_monitorEvent)) {
...
, d_pool(bdlf::BindUtil::bind(&MultiQueueThreadPool<TYPE>::eventCreator,
         ...
         config.d_growBy,
         basicAllocator)
, d_monitorEvent(Event::BMQC_MONITOR, basicAllocator)
, d_queueEmptyEvent(Event::BMQC_QUEUE_EMPTY, basicAllocator)
...
inline int MultiQueueThreadPool<TYPE>::enqueueEvent(Event* event, int queueId)
{
    ...
    return enqueueEventImp(event, queueId);
}
